...
Triggered when `$GITHUB_ACTIONS == "true"`. Questions become comments on the issue or PR. Use `${CLAUDE_PLUGIN_ROOT}/skills/agentic-loop/scripts/gh-comment.sh` — do not block on human answers during a single CI run; instead commit progress, post questions, and exit so the next run can pick up when the human replies. See `references/ci-mode.md`.
...
The single biggest failure mode of this loop is silently inventing an answer the human should have been asked. Treat every ambiguity as a question first, an assumption never. Both modes must interview — interactive via `AskUserQuestion`, CI via `${CLAUDE_PLUGIN_ROOT}/skills/agentic-loop/scripts/gh-comment.sh` with the Open-Questions template.
...
In CI mode a `PostToolUse` hook drops `.agentic-loop/eject-flag` when the token-budget heuristic trips. **At every per-task checkpoint and at the top of every review-loop iteration**, check the flag before dispatching a subagent; if present, run `git-sync.sh checkpoint`, bump the resume-attempts counter, post the bot eject comment, and `exit 0`. The `PreCompact` hook (`${CLAUDE_PLUGIN_ROOT}/skills/agentic-loop/scripts/precompact-flush.sh`) provides an independent safety net by checkpointing before any auto- or manual compaction.
...
At the very start of every CI invocation, after confirming `$AGENTIC_BRANCH` is checked out, post **exactly one** comment on the issue via `${CLAUDE_PLUGIN_ROOT}/skills/agentic-loop/scripts/gh-comment.sh`. The comment MUST include a clickable link to the branch on GitHub so reviewers can follow along.
...
9. **Post progress.** In CI mode, update the issue with a short progress comment every N tasks (configurable, default 3) via `${CLAUDE_PLUGIN_ROOT}/skills/agentic-loop/scripts/gh-comment.sh`.
...
3. **Open PR.** Render the body deterministically with `${CLAUDE_PLUGIN_ROOT}/skills/agentic-loop/scripts/render-pr-body.sh "$AGENTIC_ISSUE"` and pipe into `gh pr create --base "$AGENTIC_BASE_REF" --title "<title>" --body-file -`. The script reads `tasks.json` + `spec.md` + `plan.md` directly so the body always matches the committed state. See `references/pr-template.md`.
