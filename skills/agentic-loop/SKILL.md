...
A run can be steered with flags — pin the skills, MCP servers, and implementation subagents it uses for
*this* invocation without editing committed config:

```
[--skill a,b] [--mcp x,y] [--subagent t1,t2]
```

- **--skill** — project skills the run must consult (threaded into planner + specialist prompts, and
  forwarded to the `validating-specs` passes).
- **--mcp** — MCP servers the run must use. **Requires a wildcard-tool specialist:** most named code
  agents (`voltagent-*`, `feature-dev:code-*`) have fixed tool lists without `ToolSearch`/`mcp__`, and
  MCP-server *inheritance* grants connection access but cannot add tools to an agent's allow-list — so
  only wildcard-tool agents (`general-purpose`, `spec-to-pr:team-lead`, `claude`) can satisfy `--mcp` on
  the implementation hop. (The `validating-specs` hop is `All tools`, so it always can.)
- **--subagent** — agent types that become the **specialist roster** for this run, fully replacing
  `agents.specialists`. Planner and reviewer are untouched. Names are validated (registered +
  code-capable) before dispatch.

Precedence: **args win, config/auto-detect augment** — `dedup(flags ∪ config.required)` for skills/mcps,
full roster replacement for `--subagent`. Parsing, resolution, validation, and persistence to
`run-requirements.json` all happen once at **Stage 1 Ingest** (the single args-substitution site); this
section is documentation. Full grammar, `--subagent` validation rules, the `--mcp` compatibility gate, and the
REQUIRED TOOLING block that dispatch prompts embed live in `references/invocation-args.md`.

In headless/CI there are no typed flags (the args placeholder is empty); the same `{skills, mcps, specialists}`
set is sourced from `.agentic-loop.config.json` (`required` block + `agents.specialists`).
...
| File                 | Purpose                                                                                                                |
| -------------------- | ---------------------------------------------------------------------------------------------------------------------- |
| `run-requirements.json` | Resolved run tooling from invocation args + config: `{skills[], mcps[], specialists[]}`. **Write-once at ingest**, re-loaded (never re-parsed) on resume. See `references/invocation-args.md`. |
...
1. **Ingest.** Read `$AGENTIC_BRANCH`, `$AGENTIC_ISSUE`, `$AGENTIC_SLUG` from env — the workflow's "Pin agentic-loop branch" step has already created/checked out `feat/<issue>-<slug>`. Verify HEAD is on it. **Read `.agentic-loop.config.json` if present** (repo-specific base ref, quality gates, agent roster, risk categories — see `references/config.md`; absent ⇒ auto-detect defaults). **Resolve run tooling (fresh start only).** The invocation may carry flags — the raw string is delivered at the canonical parse site (the fenced token immediately below this list):

```
$ARGUMENTS
```

Parse `[--skill a,b] [--mcp x,y] [--subagent t1,t2]` from that line; **empty or unsubstituted ⇒ no flags** (the normal headless/CI case — tooling then comes from config, never a leaked literal). Build the resolved set `{skills, mcps, specialists}` per `references/invocation-args.md` (skills/mcps = `dedup(flags ∪ config.required)`; `specialists` = a validated `--subagent` roster if present, **fully replacing** `agents.specialists`, else config, else the default single `general-purpose`), **validate every `--subagent` name before use** (registered + code-capable per that reference; surface failures via `AskUserQuestion` interactively or `notify.sh blocked` headless — never the interview-exit path), and write `.agentic-loop/<id>/run-requirements.json`. On a **resume** (`.state` present) do NOT re-parse — re-load that file; it is the immutable run config and the args placeholder is ignored. This is the only args-substitution site; the `## Invocation Arguments` section is documentation that points here. **Pre-flight probe:** run `gh auth status`, `git status`, and `which jq` once. Any failure → follow `BLOCKED_PERMISSION` immediately (see `references/status-handling.md`); do NOT enter Stage 1 work. Quality gates are resolved per-repo by `scripts/quality-gates.sh` (config or auto-detect) — there is no single required package manager. If `.agentic-loop/<id>/.state` exists, this is a **resume** — read it, post the Resume Banner, and skip directly to the indicated stage. Otherwise: read the source (issue body via `gh issue view $ISSUE`, spec file, or inline task card), save raw content to `.agentic-loop/<id>/source.md`, then `${CLAUDE_PLUGIN_ROOT}/skills/agentic-loop/scripts/git-sync.sh commit "chore(loop): ingest source + run config for #<issue>"` (this single ingest commit also captures the `run-requirements.json` written just above).
2. **Set label.** `state:planning`. See `${CLAUDE_PLUGIN_ROOT}/skills/agentic-loop/scripts/gh-label.sh`.
3. **Delegate to the planner for spec planning.** Dispatch `Agent(<agents.planner>)` with `prompts/spec-planner.md` and `model: "opus"`. The planner returns a draft `spec.md` structured per its TDD format: Context · Goals · Non-goals · Architecture · Components · Test Strategy · Acceptance Criteria · Resolved Decisions · Open Questions · Risks. If the spec touches domain-specific research (domain mechanics, external APIs), the planner will include a `## Research Needed` section — **the controller** then dispatches the relevant configured specialist (`agents.specialists`, or `general-purpose`), **prepending the REQUIRED TOOLING block from `run-requirements.json`** (`references/invocation-args.md`) so research honours the run's required skills/MCPs, and passes findings back in the next spec draft iteration. The planner never dispatches specialists itself.
...
6. **Review — two passes.** Both run every iteration; neither replaces the other.
   1. **Structured review (drives the interview gate).** Dispatch `code-reviewer` with `prompts/spec-reviewer.md` and `model: "sonnet"`. It checks: placeholders, contradictions, ambiguity, missing test strategy, scope creep, requirements not traced to components. Output → `spec-review.md` with `{critical, important, minor}` lists **plus** the interview-driving fields (`open_questions`, `product_decisions_flagged`, `force_interview`). Save the reviewer's verdict **verbatim as the JSON object it returns** — never reformat it to prose or markdown, because the Stage 1→2 gate hook (`agentic-loop-check-state-transition.sh`) parses `verdict` and `force_interview` out of this file and will block the `plan` transition if it cannot read them. This pass is the source of the interview machinery — never skip it.
   2. **Validation pass (deeper pressure-test).** Invoke the `spec-to-pr:validating-specs` skill via the **Skill tool from this controller** (see _Spec & Plan Validation_ below for the dispatch contract and why it must not be a subagent). Pass the explicit `spec.md` path, **forward the run's `--skill`/`--mcp` from `run-requirements.json`** (never `--subagent`) so the validation instances require the same tooling this run mandates, and direct its merged report to `.agentic-loop/<id>/spec-validation.md` — **not** `spec-review.md`, which the structured pass owns. It returns one `GO | REVISE | NO-GO` verdict plus a Blocker/Major/Minor findings ledger. Then `${CLAUDE_PLUGIN_ROOT}/skills/agentic-loop/scripts/git-sync.sh commit "chore(loop): spec validation for #<issue>"`.
...
1. **Dispatch plan.** Dispatch `Agent(<agents.planner>)` with `prompts/impl-planner.md` (Mode A) — filling its `<roster>` block from `run-requirements.json`'s resolved `specialists` (a validated `--subagent` override when present, else `agents.specialists`, else the default single `general-purpose`; see `references/invocation-args.md` for the bare-type → `{type, owns}` adaptation, including the multi-type `owns:"code (planner allocates)"` placeholder) — plus `spec.md` and `model: "sonnet"`. The planner returns a `<dispatch_plan>` block conforming to the **Dispatch Plan Schema** in `references/plan-format.md`. The controller validates the JSON against that schema (non-empty `specialists`, every `agent_type` in the configured roster, no cycles in `depends_on`, every `research_needed[*].feeds_into` references a known specialist). If validation fails, re-dispatch the planner with the validation error. The planner does NOT dispatch specialists itself.
2. **Controller dispatches specialists.** Using the validated `<dispatch_plan>`, dispatch any `research_needed` agents first (sequentially or in parallel depending on `feeds_into` dependencies), then dispatch the `specialists` **in parallel** (one message, multiple Agent calls), each prepended with `shared_context`. Each specialist receives: full `spec.md`, their scoped components, research findings if applicable, and `references/plan-format.md`. **Prepend the REQUIRED TOOLING block** (`references/invocation-args.md`) built from `run-requirements.json`'s `skills`/`mcps` — each skill to invoke via the Skill tool with its reason, each MCP with its ToolSearch-first line — but emit an MCP line only when the specialist is a wildcard-tool agent that can actually load it (the `--mcp` compatibility gate; a warning was already surfaced at ingest if `--mcp` co-occurs with a restricted `--subagent`). Each returns a domain-scoped TDD plan. Do NOT pass `model:` on these dispatches — let each specialist inherit its frontmatter default.
...
7. **Forward the run's required tooling.** Append the resolved `--skill`/`--mcp` from `run-requirements.json` to the invocation string (e.g. `.agentic-loop/<id>/spec.md --skill a,b --mcp x,y`) so the validation pass requires the same skills/MCPs the run mandates. **Never forward `--subagent`** — the specialist roster is implementation-only; `validating-specs` owns its own instance fan-out. `validating-specs` is `All tools`, so it can satisfy `--mcp` even when a restricted implementation specialist cannot (see the `--mcp` compatibility gate in `references/invocation-args.md`).
