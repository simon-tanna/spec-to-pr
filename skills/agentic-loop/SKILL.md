...
name: agentic-loop
description: >-
  Use when the user provides a spec, task card, GitHub issue, or feature
  description and wants autonomous end-to-end implementation; when they say
  things like "run the agentic loop", "implement this issue", "ralph this", or
  "take this from spec to PR"; or when a GitHub Action invokes the agent with an
  issue payload. Strongly prefer this skill over ad-hoc implementation whenever a
  spec can be handed off end-to-end, even if the loop is not named explicitly.
...
Triggered when `$GITHUB_ACTIONS == "true"`. Questions become comments on the issue or PR. Use `.claude/skills/agentic-loop/scripts/gh-comment.sh` — do not block on human answers during a single CI run; instead commit progress, post questions, and exit so the next run can pick up when the human replies. See `references/ci-mode.md`.
...
The single biggest failure mode of this loop is silently inventing an answer the human should have been asked. Treat every ambiguity as a question first, an assumption never. Both modes must interview — interactive via `AskUserQuestion`, CI via `.claude/skills/agentic-loop/scripts/gh-comment.sh` with the Open-Questions template.
...
Everything durable lives under `.agentic-loop/<issue-id-or-slug>/` in the repo root. The directory is **tracked in git** on the deterministic feature branch (`feat/<issue>-<slug>`). State is durable only after it has been committed AND pushed via `.claude/skills/agentic-loop/scripts/git-sync.sh commit "..."` — every state mutation in the skill goes through that helper. No bare `git commit` / `git push` calls anywhere in this skill.
...
1. **Persist eagerly.** Every state mutation (write to `spec.md`, `spec-validation.md`, `plan.md`, `plan-validation.md`, `tasks.json`, `.state`, `progress.log`, `open-questions.md`) is followed by `.claude/skills/agentic-loop/scripts/git-sync.sh commit "<message>"`. Never queue multiple state mutations and commit at the end.
2. **Checkpoint at every pause.** Before any deliberate exit (interview handshake, blocked escalation, end of stage), and at every stage boundary, call `.claude/skills/agentic-loop/scripts/git-sync.sh checkpoint "<reason>"`. This is empty-diff tolerant.

## Context Hygiene

In CI mode a `PostToolUse` hook drops `.agentic-loop/eject-flag` when the token-budget heuristic trips. **At every per-task checkpoint and at the top of every review-loop iteration**, check the flag before dispatching a subagent; if present, run `git-sync.sh checkpoint`, bump the resume-attempts counter, post the bot eject comment, and `exit 0`. The `PreCompact` hook (`.claude/skills/agentic-loop/scripts/precompact-flush.sh`) provides an independent safety net by checkpointing before any auto- or manual compaction.
...
At the very start of every CI invocation, after confirming `$AGENTIC_BRANCH` is checked out, post **exactly one** comment on the issue via `.claude/skills/agentic-loop/scripts/gh-comment.sh`. The comment MUST include a clickable link to the branch on GitHub so reviewers can follow along.

Compute the branch URL:
...
4. **Interview.** Collect questions from BOTH `open_questions[]` AND from `product_decisions_flagged[where !authorised_by_source]` in the spec-review.md. Also check `force_interview` — if true, an interview is mandatory even if both arrays are empty (this should not happen if spec-planner followed Step 0 correctly). In interactive mode, ask via `AskUserQuestion` (one at a time when ambiguous, batch when independent). In CI mode, post them as a single combined comment on the issue via `.claude/skills/agentic-loop/scripts/gh-comment.sh`, write them to `open-questions.md`, **write `.state` = `spec` to `.agentic-loop/<id>/.state`** (mandatory — even a fresh-start run must write this file so the next trigger can resume; do not rely on `.state` absence as a proxy for "still in spec"), commit progress, and exit — the next CI trigger (on human reply) resumes. See `references/ci-mode.md §Interview Handshake` for the full gate logic.
...
8. **Exit.** Zero critical + zero important in `spec-review.md` **AND** `spec-validation.md` verdict is `GO` = spec locked. Update `.state` → `plan`. Run `.claude/skills/agentic-loop/scripts/git-sync.sh commit "chore(loop): lock spec for #<issue>"`.
...
5. **Review — two passes.** Both run every iteration.
   ...
   2. **Validation pass.** Invoke the `validating-specs` skill via the **Skill tool from this controller** (see _Spec & Plan Validation_ below). Pass the explicit `plan.md` path as the spec-under-review plus `spec.md` for context, and direct its merged report to `.agentic-loop/<id>/plan-validation.md`. It returns one `GO | REVISE | NO-GO` verdict. Then `.claude/skills/agentic-loop/scripts/git-sync.sh commit "chore(loop): plan validation for #<issue>"`.
...
7. **Exit.** Update `.state` → `implement`. Set label `state:implementing`. Run `.claude/skills/agentic-loop/scripts/git-sync.sh commit "chore(loop): lock plan for #<issue>"`.
...
1. **Invoke via the Skill tool from this controller, never as a subagent.** `validating-specs` is itself a top-level controller that dispatches `spec-design-validator` subagents. Claude Code forbids nested subagent dispatch, so it MUST run inline in the main session. You (the agentic-loop controller) ARE the main session — `Skill(validating-specs)` runs inline and its `spec-design-validator` Agent calls are ordinary main-session dispatches. Dispatching `validating-specs` itself through `Agent(...)` would break it. This is the same main-session constraint that governs your own existence.
...
0. **Context hygiene check.** In CI mode, check the eject flag before starting each task (see _Context Hygiene_ above). If set, flush and exit.
...
8. **Update state.** Mark task `done` in `tasks.json` only after both `spec_review_sha` AND `quality_review_sha` are populated and quality gates passed. Record `commit_sha`. Append learnings to `progress.log` per `references/progress-journal.md`. If a reusable pattern emerged, update the relevant `CLAUDE.md`. Run `.claude/skills/agentic-loop/scripts/git-sync.sh commit "chore(loop): finish <task-id> [#<issue>]"` so state files land on origin.
9. **Post progress.** In CI mode, update the issue with a short progress comment every N tasks (configurable, default 3) via `.claude/skills/agentic-loop/scripts/gh-comment.sh`.
...
2. **Pre-PR gates.** Re-run `.claude/skills/agentic-loop/scripts/quality-gates.sh` on the full branch. Must be green.
3. **Open PR.** Render the body deterministically with `.claude/skills/agentic-loop/scripts/render-pr-body.sh "$AGENTIC_ISSUE"` and pipe into `gh pr create --base "$AGENTIC_BASE_REF" --title "<title>" --body-file -`. The script reads `tasks.json` + `spec.md` + `plan.md` directly so the body always matches the committed state. See `references/pr-template.md`.
4. **Label.** `state:in-pr`. Remove `state:implementing`.
5. **Reset auto-resume counter.** `rm -f ".agentic-loop/${AGENTIC_ISSUE}/resume-attempts"` then `.claude/skills/agentic-loop/scripts/git-sync.sh commit "chore(loop): reset resume-attempts on PR open for #${AGENTIC_ISSUE}"`. Otherwise a future re-run on the same issue inherits the cap immediately. See `references/auto-resume-config.md`.
...
- **Verify before emitting `<loop-state>COMPLETE</loop-state>`.** Run `gh pr list --head "$AGENTIC_BRANCH" --state open --json number --jq 'length'` and confirm it returns `≥ 1`. If `gh pr create` returned non-zero or the count is zero, do **not** emit COMPLETE — escalate via `.claude/skills/agentic-loop/scripts/gh-comment.sh` and exit non-zero so the workflow's failure-comment branch fires.
...
- **Scripts:** `.claude/skills/agentic-loop/scripts/quality-gates.sh` (resolves + runs the repo's quality gates — config or auto-detect; see `references/config.md`), `.claude/skills/agentic-loop/scripts/check-substrate.sh` (probe: are the gate hooks registered, i.e. deterministic vs model-only enforcement?), `.claude/skills/agentic-loop/scripts/git-sync.sh` (only place that mutates git state from the loop), `.claude/skills/agentic-loop/scripts/gh-comment.sh` (auto-prepends `@$AGENTIC_AUTHOR` — do not double-tag), `.claude/skills/agentic-loop/scripts/gh-label.sh`, `.claude/skills/agentic-loop/scripts/render-pr-body.sh`, `.claude/skills/agentic-loop/scripts/precompact-flush.sh`, `.claude/skills/agentic-loop/scripts/postooluse-context-check.sh`.
